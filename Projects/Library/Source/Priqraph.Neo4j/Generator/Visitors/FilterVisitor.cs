...
public class FilterVisitor : DatabaseVisitor<Neo4jFilterQueryFragment>
{
    ...
    protected override Neo4jFilterQueryFragment VisitAndAlso(BinaryExpression expression)
    {
    ...
    protected override Neo4jFilterQueryFragment VisitOrElse(BinaryExpression expression)
    {
        var left = Visit(expression.Left);
        var right = Visit(expression.Right);
    ...
    protected override Neo4jFilterQueryFragment VisitConvert(UnaryExpression expression, string? memberName = null)
    {
    ...
    protected override Neo4jFilterQueryFragment VisitNot(UnaryExpression expression)
    {
        var left = Visit(expression.Operand);
        var right = Neo4jFilterQueryFragment.Create(FilterProperty.CreateParameterClause(null, null, null));
        ...
        return Neo4jFilterQueryFragment.Create(FilterProperty.CreateWhereClause(left.Parameter!, right.Parameter, ConditionOperatorType.Not, clauseType));
    }

    protected override Neo4jFilterQueryFragment VisitGreaterThan(BinaryExpression expression)
    {
        var whereClause = VisitBinary(expression, ConditionOperatorType.GreaterThan);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitGreaterThanOrEqual(BinaryExpression expression)
    {
        var whereClause = VisitBinary(expression, ConditionOperatorType.GreaterThanEqual);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitLessThan(BinaryExpression expression)
    {
        var whereClause = VisitBinary(expression, ConditionOperatorType.LessThan);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitLessThanOrEqual(BinaryExpression expression)
    {
        var whereClause = VisitBinary(expression, ConditionOperatorType.LessThanEqual);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitEndsWith(MethodCallExpression expression)
    {
        var whereClause = VisitCall(expression, ConditionOperatorType.RightLike);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitStartsWith(MethodCallExpression expression)
    {
        var whereClause = VisitCall(expression, ConditionOperatorType.LeftLike);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitContains(MethodCallExpression expression)
    {
        var whereClause = VisitCall(expression, ConditionOperatorType.Contains);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitInclude(MethodCallExpression expression, bool condition)
    {
        var whereClause = VisitCall(expression, condition ? ConditionOperatorType.In : ConditionOperatorType.NotIn);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitCheckValue(MethodCallExpression expression, bool condition)
    {
        var whereClause = VisitCall(expression, condition ? ConditionOperatorType.IsNotNull : ConditionOperatorType.IsNull);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitNotEqual(BinaryExpression expression)
    {
        var whereClause = VisitBinary(expression, ConditionOperatorType.NotEqual);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitEqual(BinaryExpression expression)
    {
        var whereClause = VisitBinary(expression, ConditionOperatorType.Equal);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitEqual(MethodCallExpression expression)
    {
        var whereClause = VisitCall(expression, ConditionOperatorType.Equal);
        return Neo4jFilterQueryFragment.Create(whereClause.Parameter);
    }

    protected override Neo4jFilterQueryFragment VisitParameter(ParameterExpression expression)
    {
        ...
        return Neo4jFilterQueryFragment.Create(new FilterProperty(field, clauseType: field.AggregateFunctionType != AggregateFunctionType.None ? ClauseType.Having : ClauseType.Where));
    }

    protected override Neo4jFilterQueryFragment VisitMember(MemberExpression expression)
    {
    ...
    protected override Neo4jFilterQueryFragment VisitConstant(ConstantExpression expression, string? memberName = null, MemberExpression? memberExpression = null)
    {
        memberName ??= "param";
        var value = GetValue(expression, memberExpression);

        return Neo4jFilterQueryFragment.Create(FilterProperty.CreateParameterClause(value, GetValueType(value), memberName));
    }
    ...
    private Neo4jFilterQueryFragment VisitCall(MethodCallExpression expression, ConditionOperatorType operatorType = ConditionOperatorType.And)
    {
        Neo4jFilterQueryFragment? left;
        Neo4jFilterQueryFragment? right;
    ...
    private Neo4jFilterQueryFragment VisitBinary(BinaryExpression expression, ConditionOperatorType operatorType = ConditionOperatorType.And)
    {
        Neo4jFilterQueryFragment? left = null;
        Neo4jFilterQueryFragment? right;
        MemberExpression? memberExpression = null;
        ...
        switch (operatorType)
        {
            case ConditionOperatorType.Equal:
                ...
                {
                    right = Neo4jFilterQueryFragment.Create(FilterProperty.CreateParameterClause(null, null, null));
    ...
    private static Neo4jFilterQueryFragment CreateFilterClause(ConditionOperatorType operatorType, Neo4jFilterQueryFragment right, Neo4jFilterQueryFragment left)
    {
        ...
        return Neo4jFilterQueryFragment.Create(FilterProperty.CreateWhereClause(left.Parameter, right.Parameter, operatorType, clauseType));
    }
